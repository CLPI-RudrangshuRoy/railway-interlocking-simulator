...
public class StationA_RRI_Interlocking {

    // ===== Section and Crossover Classes =====
    static class Section {
        String name; char setting='F';
    ...
    static class Crossover {
        String name; char position='N', setting='F';
    ...
    static class AxleCounterSection {
        String name; int inCount=0,outCount=0; boolean outBeforeIn=false; Section section; JLabel inLabel,outLabel;
        ...
        boolean checkSPAD=false,checkWDTF=false,checkTP=false; // Flags
    ...
    static void logOperation(StringBuilder log,String msg){
        ...
        try(FileWriter fw=new FileWriter("StationA_RRI_Log.txt",true)){fw.write(entry+"\n");} catch(IOException e){e.printStackTrace();}
    }
    ...
    public static void main(String[] args){
        JFrame frame=new JFrame("Station A RRI Interlocking"); frame.setSize(1600,800); frame.setLayout(new BorderLayout());
        ...
        JPanel rriPanel=new JPanel(); rriPanel.setLayout(new GridLayout(8,1,5,5));
        rriPanel.setBorder(BorderFactory.createTitledBorder("RRI Options"));
        ...
        for(int i=0;i<7;i++){
            ...
            btn.addActionListener(e->applyRRIOption(opt,signalPanel,SC_HSF,SC_SSF,SC_LL,ML_HSF,LL_HSF,ML_Ad_S,log,axles));
            rriPanel.add(btn);
        }

        frame.add(rriPanel,BorderLayout.EAST);
    ...
    static void applyRRIOption(int opt,SignalPanel s,Crossover SC_HSF,Crossover SC_SSF,Crossover SC_LL,
        ...
        switch(opt){
            case 1 -> {} 
            case 2 -> { s.setHomeYellow(true); s.setMLRed(true); SC_HSF.forceSetting('F',log); ML_HSF.forceSetting('S',log);}
            case 3 -> { s.setHomeYellow(true); s.activateAllHomeFeathers(true); s.setLLRed(true); SC_HSF.forceSetting('S',log); LL_HSF.forceSetting('S',log);}
        ...
        logOperation(log,"RRI Option "+opt+" applied");
    }
